#!/usr/bin/env python3
"""Comic Fry bad-luck song, then landscape H3 music video."""
...
LOG = ROOT / "fry_badluck_mv.log"
OUTPUTS = Path("/home/ina/pinokio/api/Maestro-next.git/app/outputs")

STYLE = (
    "comic sad-sack sitcom song, Futurama cartoon vibe, light brass and slapstick "
    "percussion, wry male vocal, slightly unlucky but funny, Spanish lyrics, "
    "no rap, no trap, no pop ballad, no concert stage"
)
LYRICS = """[Estrofa]
Pidió una pizza y se durmió mil años
el trabajo le espera, el trabajo ya es otro
se tropieza en la nave, se cae en el tubo
el universo se ríe, Fry no tuvo ni un punto
...
[Estrofa]
La paloma le apunta, el café se le vuelca
Bender le bebe la lata, Leela pone los ojos
gana el premio y lo pierde, se ata los cordones
y el futuro entero le pisa los calcetines
...
"""
SCENE = (
    "Videoclip landscape 16:9 de Philip J. Fry de Futurama. Dibujo 2D de serie: "
    "piel anaranjada, pelo naranja, chaqueta roja, camiseta blanca, pantalón azul, "
    "no live action, no fotorealismo 3D. Escenas cómicas de mala suerte cotidiana: "
    "se tropieza, se le cae el café, paloma, criogenización, entrega de pizza mil años tarde, "
    "cae por una alcantarilla, pulsa el botón equivocado, Bender le quita la cerveza. "
    "New New York, Planet Express, tono slapstick. Fry es el protagonista. "
    "Nadie canta a cámara de rapero. Sin MCs, hoodies, cadenas ni escenario de concierto. "
    "Narrativa cómica, coros en off, 16:9."
)
...
def main() -> None:
    ...
    (ROOT / "fry_badluck_song.json").write_text(
    ...
    pipeline = request_json("POST", "/api/v1/director/pipeline/start", {
        ...
        "music_video_treatment": {
            ...
            "forbidden_elements": (
                ...
                "live-action photoreal Fry, contemporary streetwear"
            ),
            "direct_video_master_prompt": (
                "Dibujo 2D estilo Futurama, 16:9 landscape: New New York, "
                "Planet Express, Fry de chaqueta roja teniendo mala suerte cómica. "
                "Slapstick, no drama. Ningún rapero moderno."
            ),
        },
    ...
    (ROOT / "fry_badluck_pipeline.json").write_text(
        json.dumps({
            "source": source,
            "audio_path": audio_path,
            "pipeline": pipeline,
    ...
    log("fry bad-luck music video done")
