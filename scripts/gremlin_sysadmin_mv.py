#!/usr/bin/env python3
"""Sysadmin-gremlins song, then landscape H3 music video."""
...
LOG = ROOT / "gremlin_sysadmin_mv.log"
OUTPUTS = Path("/home/ina/pinokio/api/Maestro-next.git/app/outputs")

STYLE = (
    "80s Joe Dante Gremlins mischief score meets datacenter industrial, "
    "playful chaotic choir of small creatures, synth and tape hiss, "
    "server-room percussion of fans and keyboards, Spanish lyrics, "
    "no rap, no pop, no trap, no modern concert"
)
LYRICS = """[Estrofa]
Tres reglas en el rack, tres reglas en la piel
no mojes el mogwai, no riegues el panel
después de medianoche no hay deploy ni miel
la luz del CPD nos vuelve otra piel
...
[Estrofa]
Gizmo en la guardia, Stripe en root
cables como nidos, el sudo no perdona
un ticket a las tres, otro en la consola
parpadean los leds, la cinta no perdona
...
"""
SCENE = (
    "Videoclip landscape 16:9: sysadmins que SON gremlins de la película Gremlins (1984), "
    "Joe Dante / Chris Walas. Criaturas pequeñas, peludas o verdes con dientes, "
    "no humanos disfrazados, no raperos. Sala de servidores de los 80-90: racks, "
    "leds, cintas, café, cables, pantallas verdes, fluorescente. "
    "Gizmo mogwai de guardia; Stripe y la manada cuando alguien vierte agua o "
    "despliega después de medianoche. Narrativa caótica de CPD, coros en off, 16:9. "
...
def main() -> None:
    ...
    (ROOT / "gremlin_sysadmin_song.json").write_text(
    ...
    pipeline = request_json("POST", "/api/v1/director/pipeline/start", {
        ...
        "music_video_treatment": {
            ...
            "forbidden_elements": (
                ...
                "human sysadmins without gremlin anatomy, contemporary streetwear"
            ),
            "direct_video_master_prompt": (
                "Cine 80s tipo Gremlins de Joe Dante, 16:9 landscape: "
                "sala de servidores, leds, café, criaturas pequeñas con dientes. "
                "Los gremlins SON los sysadmins. Ningún rapero moderno."
            ),
        },
    ...
    (ROOT / "gremlin_sysadmin_pipeline.json").write_text(
        json.dumps({
            "source": source,
            "audio_path": audio_path,
            "pipeline": pipeline,
    ...
    log("gremlin sysadmin music video done")
