...
**Generate all pending 2D shots** runs the existing Scene Animator automatically: prepare source-keyed transparent cutouts, generate each dialogue line with the exact linked Character Kit voice, measure its audio, extend the shot when needed, save editable keyframes and audio, export, then import a completed but unapproved take. It skips existing drafts/finals and active attempts. A visible batch banner offers stop-after-current-shot. Keep the browser tab open; completed takes persist independently and retry processes only missing takes. This is basic limited-animation blocking. Dialogue preflight requires the exact linked kit's approved base and four compatible, approved mouth states with saved placement. Missing setup links to Character Creator. The compositor mounts mouth overlays parented to the exact Series character layer and compiles held keyframes with its existing known-text planner, timed within measured phrase spans (not phonetic audio alignment). A wiped base uses its own source-keyed transparent derivative; the original image and its cutout cannot replace the mouthless pose. The batch does not create mouth rigs or complex acting.

**Update shots after character changes** is a collapsed action group shown when 2D takes already exist. It selects completed 2D takes with saved editable scenes whose mouth inputs changed or were never applied. It reuses the recorded audio and authored motion, rejects mismatched dialogue or scene ownership, and appends a new unapproved take. Approved takes and active jobs are excluded. The shared update plan splits ready shots from those awaiting configuration, so blocked subjects do not stop unrelated shots. Only update candidates contribute blockers. Character links explain missing workshop base/mouth approvals. Initial generation keeps its own visible setup links; its zero-count button is hidden once videos exist. Offscreen speakers keep audio-only dialogue beats and do not require a visible mouth kit or animate another character. A versioned fingerprint of visible speaker IDs, pose, mouths, placement and dialogue prevents duplicate unchanged retakes; each shot reads the current saved kit library. Character setup is read-only throughout production. Save the workshop, return to Series Lab and refresh configuration to see remaining prerequisites.
