...
**Update lip sync in generated takes** selects completed 2D takes with saved editable scenes whose mouth inputs changed or were never applied. It reuses the recorded audio and authored motion, rejects mismatched dialogue or scene ownership, and appends a new unapproved take. Approved takes and active jobs are excluded. A fingerprint of the speaker IDs, pose, mouths, placement and dialogue prevents duplicate unchanged retakes; each shot reads the current saved kit library. Character setup is read-only throughout production. Save the workshop, return to Series Lab and refresh configuration to see remaining prerequisites.
