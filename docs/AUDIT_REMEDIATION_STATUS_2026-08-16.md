...
| SER-02 | completado | Borrado consulta centralmente checkpoints durables planning/render/assembly y devuelve 409 estructurado con IDs activos; terminales permiten borrar sin tocar outputs. Start/resume publica `queued` bajo el lock de biblioteca para cerrar carreras con DELETE. 16 lifecycle + 8 Assembly tests y compilación verdes. |
| SER-UI-01 | completado | Job local se resetea por `episode.id`; start/poll/cancel/resume/apply y errores tardíos verifican episodio propietario, y una propuesta exige que tanto job como resultado coincidan con el episodio visible. Promesa E1 diferida tras rerender E2 no se muestra ni aplica; suite UI, lint y build verdes sobre HEAD integrado. |
...
| DUR-01 | completado | Estimador central de sílabas/pausas reutilizado; contrato persistido distingue continuo, lista discreta y lattice H3 y sólo redondea hacia arriba. `SeriesShotDurationControl` único muestra voz/clip y recalcula al editar; propuesta, reapertura, regeneración y render comparten segundos/frames exactos. Caso 6,51 s => 6,51 continuo, 10 discreto, 158 frames/6,583 H3; 137 backend + 96 UI, lint/build verdes. |
...
| SER-03 | completado | Assembly publica una tarea canónica y ofrece cancel/resume/recovery/discard por workspace; los checkpoints activos tras reinicio pasan a `interrupted` recuperable y FFmpeg recibe cancelación cooperativa. Start conserva el lock transaccional de la biblioteca, cancelación tardía elimina el output antes de publicarlo y la UI expone los controles. Contrato OpenAPI actualizado; 25 tests backend dirigidos, 98 UI, lint/build verdes. |
...
| EDITOR-01 | completado | El handoff prueba todas las fuentes en una colección temporal, confirma sobre montaje existente y sólo reemplaza estado/draft y consume la clave tras persistencia completa; fallo conserva montaje y ofrece Retry. Guardia evita doble consumo en StrictMode. 92 UI, lint y build verdes. |
| EDITOR-02 | completado | Export job ID persistido por workspace; al montar/cambiar workspace consulta status y reanuda un único polling. Guardas de envío y job activo impiden un segundo POST; cancel/terminal reconcilian storage. Desmontar/remontar conserva el mismo ID y POST único; 97 UI, lint/build verdes. |
...
| CANCEL-01 | completado | Token por job/workspace cierra únicamente la respuesta HTTP activa en streaming local/OpenAI-compatible/Anthropic; fallback informa safe boundary, stages se preservan y tokens se limpian. Harness Series carga el helper real de mensaje; 121 tests Series verdes. |
...
| MEM-01 | completado | Hook único `useObjectUrl` posee/revoca sólo blobs locales y acepta fallback persistido sin revocarlo; DirectorChat, filas de refs, DirectorPanel y previews recuperadas ya no crean URLs en render. Sustitución/unmount/fallback cubiertos; lint/build verdes. |
