...
class H3WindowPlannerTests(unittest.TestCase):
    ...
    def test_ui_and_runtime_use_explicit_prompt_arrays(self):
        ...
        studio = json.loads((ROOT / "ui" / "src" / "i18n" / "locales" / "en" / "studio.json").read_text(encoding="utf-8"))
        ...
        self.assertIn("t('advanced.planWindows')", advanced)
        self.assertEqual(studio["advanced"]["planWindows"], "Plan Prompt Across Windows")
        self.assertIn("t('prompt.h3Exact'", prompt_input)
        self.assertIn("Exact H3 prompts", studio["prompt"]["h3Exact"])
