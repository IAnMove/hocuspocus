...
def build_hf_url(repo, folder, filename):
    return f'https://huggingface.co/{repo}/resolve/{REVISION}/{folder}/{filename}'
...
class family_handler:
    ...
    def query_model_def(base_model_type, model_def):
        ...
        return {
            ...
            'inference_steps': not flash, 'temperature': False, 'supports_early_stop': False,
            'profiles_dir': ['auk'], 'compile': False,
            'single_audio_reference': True, 'literal_speech_instructions': True, 'max_voice_count': 1,
            'default_num_inference_steps': 4 if flash else 32, 'default_guidance_scale': 0 if flash else 2,
            'lock_inference_steps': flash, 'lock_guidance_scale': flash,
    ...
    def query_model_files(computeList, base_model_type, model_def=None):
        return {'repoId': REPO_ID, 'revision': REVISION, 'sourceFolderList': ['auk', ENCODER_FOLDER], 'fileList': [['vae_bf16.safetensors'], ENCODER_FILES]}
    ...
    def validate_generative_settings(base_model_type, model_def, inputs):
        if inputs.get('audio_prompt_type', '') not in ('', 'A'):
            return 'AuK supports one reference audio or instruction-only speech.'
        if not str(inputs.get('prompt', '')).strip():
            return 'AuK requires an instruction.'
