...
from .transformer_qwenimage21 import QwenImage21KVCache, QwenImage21Transformer2DModel
...
class QwenImage21Pipeline:
    ...
    def _as_vae_tensor(self, image, width, height):
        tensor = self.image_processor.preprocess(image, width=width, height=height)
        if tensor.shape[1] == 3:
            alpha = tensor.new_ones(tensor.shape[0], 1, *tensor.shape[2:])
    ...
    def _prepare_local_edit(
        ...
        if image_mask is None or source_image is None:
            return None, None, 0, timestep_count
        latent_height = 2 * (int(height) // (self.vae_scale_factor * 2))
        latent_width = 2 * (int(width) // (self.vae_scale_factor * 2))
        mask_image = image_mask.convert("L").resize((latent_width, latent_height), resample=PILImage.Resampling.LANCZOS)
        mask = torch.from_numpy(np.array(mask_image, dtype=np.float32) / 255.0)
        mask = (mask > 0.5).to(device=device, dtype=dtype).view(1, latent_height * latent_width, 1)
        source = source_image.convert("RGBA") if getattr(source_image, "mode", None) != "RGBA" else source_image
        if source.size != (width, height):
            source = source.resize((width, height), resample=PILImage.Resampling.LANCZOS)
        ...
        first_step = 0
        if strength < 1 and timestep_count > 1:
            first_step = min(timestep_count - 1, int(timestep_count * (1.0 - max(0.0, min(1.0, strength)))))
        keep = 1.0 if masking_strength is None else max(0.0, min(1.0, float(masking_strength)))
        masked_steps = max(1, math.ceil(timestep_count * keep))
        return mask, original, first_step, masked_steps
        ...
        **kwargs,
        ...
        if image is not None:
            ...
            for img in image:
                ...
                vae_images.append(self._as_vae_tensor(img, input_width, input_height))
        ...
        if original_image_latents is not None and first_step > 0:
            noise_scale = float(timesteps[first_step]) / 1000
            latents = original_image_latents * (1.0 - noise_scale) + latents * noise_scale
        ...
        if output_type == "latent":
            ...
            if image.shape[1] == 4 and output_type == "pt":
