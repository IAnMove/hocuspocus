...
for shot, number in shots:
    ...
    clips.append({'name': f'CLIP {number:02} — {shot["title"]}',
                  ...
                  'trim_end': shot.get('trim_end', shot['document'].get('duration', 0)),
...
body = {'name': args.name or plan['title'], 'workspace': args.workspace,
        'width': plan.get('width', shots[0][0]['document'].get('width', 1280)),
        'height': plan.get('height', shots[0][0]['document'].get('height', 720)),
        'fps': plan.get('fps', 30), 'clips': clips,
