...
test('switching image intent submits that draft\'s output count, not the previous batch size', () => {
  setup()
  useStore.getState().setOutputCount(4)
  assert.equal(useStore.getState().params.repeat_generation, 4)
  useStore.getState().setImageStudioIntent('edit')
  assert.equal(useStore.getState().imageStudioIntent, 'edit')
  assert.equal(useStore.getState().outputCount, 1)
  assert.equal(useStore.getState().params.repeat_generation, 1)
  useStore.getState().setParams({ image_guide: '/api/v1/uploads/source.png', prompt: 'Edit once' })
  assert.equal(snapshotStudioImageIntent(useStore.getState()).params.repeat_generation, 1)
  useStore.getState().setOutputCount(3)
  useStore.getState().setImageStudioIntent('new')
  assert.equal(useStore.getState().outputCount, 4)
  assert.equal(useStore.getState().params.repeat_generation, 4)
  useStore.getState().setParam('prompt', 'Make four')
  assert.equal(snapshotStudioImageIntent(useStore.getState()).params.repeat_generation, 4)
