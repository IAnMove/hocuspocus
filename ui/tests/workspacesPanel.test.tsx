...
test('Runs processing lists planned shots, models and a video placeholder', { concurrency: false }, async () => {
  ...
  try {
    ...
    assert.ok(screen.getByRole('button', { name: /New → old/ }))
    assert.ok(screen.getByPlaceholderText('Search run…'))
    ...
    assert.ok(screen.getByRole('button', { name: /Regenerate full video/ }))
    assert.ok(screen.getByRole('button', { name: 'Select all' }))
    assert.ok(screen.getByRole('button', { name: /Propose on selected/ }))
    assert.ok(screen.getByPlaceholderText(/remove every MC/))
    assert.ok(screen.getByText(/Queue from planned prompts/))
    assert.ok(screen.getByRole('button', { name: /More runs \(1\/12\)/ }))
  } finally {
    cleanup()
  }
})
