...
test('Dark Fantasy includes all variants, PSX narrows it, and vertical presets retain authored framing', () => {
  ...
  assert.equal(filterScene3DTemplates(input).length, 50)
  assert.equal(filterScene3DTemplates({ ...input, category: 'psx' }).length, 76)
  assert.equal(filterScene3DTemplates({ ...input, category: 'all', query: 'dark fantasy' }).length, 50)
