...
test('Character Kits explain the next click instead of library jargon', async () => {
  ...
  try {
    ...
    assert.ok(screen.getByText('Cutout characters'))
    assert.ok(screen.getByText(/A character is a body/))
    assert.ok(screen.getByText(/Place the box over the lips/))
    assert.ok(screen.getByRole('button', { name: 'Put on the scene' }))
    assert.ok(screen.getByRole('button', { name: 'Save' }))
    assert.ok(screen.getByRole('button', { name: /Go to Mouths and eyes/ }))
    assert.ok(screen.getByText('Reaction'))
