...
export function Scene3DScreenControls({ slot, meshes, nodes = meshes, items, disabled, workspace, onChange, onChoose, onRemove }: {
  ...
  return <section className="mt-3 space-y-3 rounded-lg border border-border p-3" data-testid="scene3d-screen-controls">
    <ScreenToggle slot={slot} screen={screen} nodes={nodes} meshes={meshes} disabled={disabled} onChange={onChange} />
    {screen && <>
      {slot.media === 'model3d' && <ModelScreenPlacement screen={screen} nodes={nodes} meshes={meshes} disabled={disabled} patch={patch} />}
      <ScreenMediaContent screen={screen} items={items} disabled={disabled} onChoose={onChoose} patch={patch} />
...
function ScreenMediaContent({ screen, items, disabled, onChoose, patch }: {
  ...
  const { t } = useUiTranslation('scene3dEditor')
  if (screen.poseSequence) return null
  return <>
...
function ScreenToggle({ slot, screen, nodes, meshes, disabled, onChange }: {
  slot: Scene3DSlot; screen?: MediaScreen; nodes: string[]; meshes: string[]; disabled: boolean
  onChange: (value: MediaScreen | undefined) => void
}) {
  const { t } = useUiTranslation('scene3dEditor')
  return <>
