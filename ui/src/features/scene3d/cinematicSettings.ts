...
export function parseEnvironment(raw: unknown): Scene3DDocument['environment'] {
  ...
  const floorStyle = v.floorStyle === 'mirror' || v.floorStyle === 'none' || v.floorStyle === 'tiles' || v.floorStyle === 'backdrop' || v.floorStyle === 'road' ? v.floorStyle : undefined
  ...
  const road = parseEndlessRoad(v.road)
  return { ...(road ? { road } : {}), reflectiveFloor: v.reflectiveFloor === true, platform: v.platform === true, bloom: bounded(v.bloom, .48, 0, 1.5), ...(floorStyle ? { floorStyle } : {}), ...(floorColor ? { floorColor } : {}), ...(floorSourceHeight !== undefined ? { floorSourceHeight } : {}) }
}
