...
import type { seriesShotReferences, OpenSeriesReference } from './shotReferences'
...
export function SeriesShotReferencePanel({ references, shot, busy, onChange, onOpenReferences, onOpenEpisode }: ReferenceProps & {
  ...
  return <fieldset className="space-y-3 rounded-lg border border-border p-3">
        ...
        <ul className="space-y-2 text-xs">{[
          { id: shot.locationId, room: 'locations' as const, name: `${t('production.assets.location')}: ${references.location?.name || t('production.assets.chooseLocation')}`, asset: references.background, state: references.locationState },
          ...
          <div className="min-w-0"><button className="break-words text-left text-violet-200 underline underline-offset-2" disabled={!onOpenReferences || item.state === 'missingEntity'} onClick={() => onOpenReferences?.(item.room, item.id)}>{item.name}</button><p className={item.state === 'ready' ? 'text-emerald-300' : 'text-amber-300'}>{t(`production.assets.states.${item.state}`)}</p></div>
        </li>)}</ul>
