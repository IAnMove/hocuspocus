...
export type AgentVisualState = 'idle' | 'listening' | 'thinking' | 'success' | 'error'
...
const STORAGE_PREFIX = 'hocuspocus-agent-chat-v1:'
...
const welcomeMessage = (): AgentMessage => ({
  id: newId(),
  role: 'assistant',
  text: 'Hola. Puedo explicarte cualquier parte de HocusPocus y consultar la cola real de este workspace. En este primer paso todavía no ejecuto cambios: si me pides algo, te enseñaré el plan exacto antes de conectar los controles.',
  createdAt: Date.now(),
})
  ...
  const ask = async (text: string) => {
    ...
    try {
      const answer = await generateLlmText({
        ...
        prompt: buildAgentTurnPrompt(workspace, nextMessages, tasks),
        max_new_tokens: 900,
        temperature: .25,
      })
      ...
      const assistantMessage: AgentMessage = {
        id: newId(),
        role: 'assistant',
        text: answer.trim() || 'No he recibido una respuesta utilizable del LLM configurado.',
        createdAt: Date.now(),
      }
    ...
    >
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-3 py-3" aria-live="polite">
        ...
        {busy && (
          <div className="flex items-center gap-2 text-[10px] text-amber-100/60">
            <AgentAvatar state="thinking" size={24} />
            <Loader2 size={11} className="animate-spin" /> Consultando el estado y la guía de Maestro…
          </div>
        )}
        <div ref={endRef} />
      </div>
      ...
      {messages.length <= 1 && (
        <div className="flex flex-wrap gap-1.5 border-t border-white/5 px-3 pt-2">
          {['¿Qué hay en cola?', '¿Cómo uso 3D Video?', '¿Qué puedo pedirte?'].map(suggestion => (
      ...
      <form onSubmit={submit} className="border-t border-white/10 p-3">
        ...
        <p className="mt-1.5 text-center text-[8px] text-white/30">Consulta y explica. Navegación y ejecución llegarán mediante acciones revisables.</p>
      </form>
    </section>
  )
}
