...
- Use start_director_production with confirm=true only after stage_story_video when the user explicitly asks to launch that prepared film/trailer. It starts the exact Wizard handoff, returns the real Director pipeline ID and links it to Story production history. Never claim completion at launch.
...
- There is no “Render page” control. Panel artwork is Comic Director → **Generate all images**, or generate_comic with confirm=true after “lánzalo / dibuja las viñetas”. generate_comic may set image_provider=minimax and then renders every unfinished panel without asking the user to click. Local panels share the GPU queue; MiniMax uses its configured external provider.
