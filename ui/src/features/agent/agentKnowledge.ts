...
- Qwen Image 2.1 (qwen_image_21, plus _bf16 and _gguf_* variants) is a unified text-to-image and image-edit model. Prefer an enabled qwen_image_21* id from available_image_models for stills and for editing an existing picture. There is no separate Qwen-Image-Edit-2.1 checkpoint. Native 2K is resolution_preset=1080p (or resolution=2048x2048). Do not use Studio → Edit (that tab is video).
- To edit an existing still with Qwen 2.1: prepare_image with that model and the change described as the finished picture, then attach_studio_references with reference_role=edit_source (the canvas to change). Optional reference_role=edit_mask is a white=change mask. Optional outpaint_margins is "top bottom left right" percents and needs an edit_source. Identity/style still uses subject or style (up to max_image_refs, 10 on 2.1). Put attach_studio_references after prepare_image and before start_generation.
...
- Use remove_background with confirm=true for an exact image/video asset or canonical source. It opens Tools, creates a transparent derived PNG for images or WebM for videos (up to 60 seconds / 1800 frames, preserving audio) through the shared rembg/U2Net adapter and records source lineage in the normal library; never invent asset IDs or claim completion before Activity reports it.
- Use attach_studio_references only with exact names from recent_image_outputs. Put it after prepare_image/prepare_video and before start_generation in the same turn. reference_role=start_frame is I2V; subject preserves people/objects; style preserves subject/landscape style; edit_source is the Qwen 2.1 canvas to change; edit_mask is its local mask. Never invent a filename.
