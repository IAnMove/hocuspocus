...
export function WorkspaceCollectionsPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : tWs('collections.loadFailed'))
  ...
  return (
    <section aria-label={tWs('collections.aria')} className="flex h-full min-h-0 overflow-hidden rounded-xl border border-border bg-bg-primary">
      ...
      <div className="min-w-0 flex-1 overflow-y-auto p-4">
        <div className="mb-3 flex items-center justify-between gap-2">
          <div><h2 className="text-sm font-semibold text-text-primary">{draft?.name || tWs('collections.untitled')}</h2><p className="text-[10px] text-text-muted">{tWs('collections.idsHint')}</p></div>
