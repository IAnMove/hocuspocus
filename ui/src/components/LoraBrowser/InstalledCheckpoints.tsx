    ...
    <div className="h-full overflow-y-auto p-4">
      ...
      <div className="flex items-center gap-2 mb-4">
        <span className="text-xs text-text-muted">
          {t('loraBrowser.checkpointsCount', { count: items.length })}
          {updatableCount > 0 && (
            <span className="ml-2 text-indicator-warning">{t('loraBrowser.updatesAvailable', { count: updatableCount })}</span>
          )}
        </span>
        {lastCheck && (
          <span className="text-[10px] text-text-muted">{t('loraBrowser.checked', { date: new Date(lastCheck).toLocaleDateString() })}</span>
        )}
        <button
          onClick={handleCheck}
          ...
          title={t('loraBrowser.checkCheckpoints')}
        >
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-text-muted">
          ...
          <p className="text-sm">{t('loraBrowser.noCheckpoints')}</p>
          <p className="text-xs mt-1">{t('loraBrowser.switchToSearch')}</p>
        </div>
      ) : (
            ...
            return (
              <button
                ...
                title={c.update_status === 'available'
                  ? t('loraBrowser.updateOpen')
                  : (clickable ? t('loraBrowser.openCivitai') : undefined)}
              ...
              >
                <div className="aspect-[3/4] bg-bg-active overflow-hidden">
                  {c.preview_url ? (
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center text-text-muted gap-1">
                      ...
                      <span className="text-[9px] opacity-40">{t('loraBrowser.noPreview')}</span>
                    </div>
                  )}
                </div>
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/50 to-transparent p-2 pt-6">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                    ...
                    {c.auto_quantize && (
                      <span className="flex items-center gap-0.5 text-[9px] text-white/60" title={t('loraBrowser.int8Title')}>
                        <Cpu size={8} /> int8
                      </span>
                    )}
                  ...
                  <div className="absolute top-1.5 left-1.5">
                    <span
                      ...
                      title={c.latest_published_at
                        ? t('loraBrowser.updatePublished', { date: new Date(c.latest_published_at).toLocaleDateString() })
