  ...
  return (
    <div className="block text-[9px] text-text-muted">
      ...
      <div className="mt-0.5 flex w-full items-center gap-2 rounded border border-border bg-bg-primary px-1.5 py-1 text-left text-[10px] text-text-primary">
        {selected?.type === 'image' && preview ? <ImagePreview image={{ ...selected, url: localEditPreview(selected.url), type: 'image' }} className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded bg-bg-active">
          <InputImageThumbnail item={selected} />
