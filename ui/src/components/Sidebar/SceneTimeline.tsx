  ...
  return <div className="mt-3 overflow-hidden rounded-lg border border-border bg-bg-secondary">
    ...
    {selectedLayer && selectedTiming && <div className="border-t border-border bg-bg-tertiary px-2 py-2">
      ...
      <div className="grid grid-cols-2 gap-1.5 md:grid-cols-5">
        {numberField('Offset', selectedTiming.offset, value => onUpdateTiming({ offset: Math.max(0, value) }), .05, 0, duration, locked)}
        {numberField('Trim in', selectedTiming.trimStart, value => onUpdateTiming({ trimStart: value }), .05, 0, selectedLayer.animation.duration - .01, locked)}
        {numberField('Trim out', selectedTiming.trimEnd, value => onUpdateTiming({ trimEnd: value }), .05, .01, selectedLayer.animation.duration, locked)}
        {numberField('Speed', selectedTiming.speed, value => onUpdateTiming({ speed: value }), .1, .1, 8, locked)}
        <label className="flex items-end gap-1.5 pb-1 text-[9px] text-text-secondary"><input type="checkbox" checked={selectedTiming.loop} disabled={locked} onChange={event => onUpdateTiming({ loop: event.target.checked })} /> Repeat motion</label>
      </div>
    ...
    {selectedFrame && selectedLayer && <div className="border-t border-border bg-bg-tertiary px-2 py-2">
      ...
      <div className="grid grid-cols-3 gap-1.5 md:grid-cols-6">
        {numberField('Time', selectedFrame.time, value => onUpdateKeyframe(selectedFrame.id, { time: value }), .05, 0, duration, isEndpoint || locked)}
        {numberField('X', selectedFrame.x, value => onUpdateKeyframe(selectedFrame.id, { x: value }), .5, -100, 200, locked)}
        {numberField('Y', selectedFrame.y, value => onUpdateKeyframe(selectedFrame.id, { y: value }), .5, -100, 200, locked)}
        {numberField('Scale', selectedFrame.scale, value => onUpdateKeyframe(selectedFrame.id, { scale: Math.max(.01, value) }), .05, .01, 10, locked)}
        {numberField('Opacity', selectedFrame.opacity, value => onUpdateKeyframe(selectedFrame.id, { opacity: Math.max(0, Math.min(1, value)) }), .05, 0, 1, locked)}
        {numberField('Rotation', selectedFrame.rotation, value => onUpdateKeyframe(selectedFrame.id, { rotation: value }), 1, -1080, 1080, locked)}
      </div>
      <div className="mt-2 flex items-end gap-2">
        <label className="min-w-36 text-[9px] text-text-muted">Easing to next keyframe<select value={selectedFrame.curve} disabled={isLast || locked} onChange={event => onUpdateKeyframe(selectedFrame.id, { curve: event.target.value as SceneCurve })} className="mt-0.5 w-full rounded border border-border bg-bg-primary px-1.5 py-1 text-[10px] disabled:opacity-50">{CURVES.map(curve => <option key={curve} value={curve}>{curve}</option>)}</select></label>
