...
type Preset = { id: string; label: string; start: Point; end: Point; duration: number; spin: boolean; curve: SceneCurve }
...
].map(([id, label, sx, sy, ex, ey, ss, es, duration, spin, curve]) => ({ id: id as string, label: label as string, start: makePoint(sx as number, sy as number, ss as number), end: makePoint(ex as number, ey as number, es as number), duration: duration as number, spin: spin as boolean, curve: curve as SceneCurve }))
...
export function SceneAnimatorPanel() {
  ...
  const layerState = (layer: SceneLayer, time = progress) => { const t = ease(Math.min(1, time * scene.duration / Math.max(1, layer.animation.duration)), layer.animation.curve); return { x: lerp(layer.animation.start.x, layer.animation.end.x, t), y: lerp(layer.animation.start.y, layer.animation.end.y, t), scale: lerp(layer.animation.start.scale, layer.animation.end.scale, t), opacity: lerp(layer.animation.start.opacity ?? layer.transform.opacity, layer.animation.end.opacity ?? layer.transform.opacity, t), rotation: layer.transform.rotation ?? 0 } }
  ...
  const applyPreset = (presetId: string) => { if (!selected) return; const preset = PRESETS.find(item => item.id === presetId); if (!preset) return; updateLayer(selected.id, layer => ({ ...layer, animation: { start: preset.start, end: preset.end, duration: preset.duration, curve: preset.curve, spin: preset.spin, rotationSpeed: layer.animation.rotationSpeed } })); setProgress(0) }
  const motion = (layer: SceneLayer) => ({ start: layer.animation.start, end: layer.animation.end, duration: layer.animation.duration, curve: layer.animation.curve, spin: layer.animation.spin, rotationSpeed: layer.animation.rotationSpeed })
  ...
  const paintScene = (canvas: HTMLCanvasElement, time: number) => {
    ...
    scene.layers.filter(layer => layer.visible).sort((a, b) => a.z - b.z).forEach(layer => {
      const state = layerState(layer, time); context.save(); context.globalAlpha = state.opacity
  ...
  const renderLayer = (layer: SceneLayer) => {
    const state = layerState(layer); const common = { left: `${state.x}%`, top: `${state.y}%`, width: `${(layer.type === 'model3d' ? 52 : 100) * state.scale}%`, height: `${(layer.type === 'model3d' ? 75 : 100) * state.scale}%`, opacity: state.opacity, zIndex: layer.z, transform: `translate(-50%, -50%) rotate(${state.rotation}deg)` }; const selection = selectedId === layer.id
