...
function sharedStableSerialize(value: unknown): string {
  if (value == null || typeof value !== 'object') return JSON.stringify(value ?? null)
  if (Array.isArray(value)) return `[${value.map(item => sharedStableSerialize(item)).join(',')}]`
  const entries = Object.entries(value as Record<string, unknown>)
    .filter(([, item]) => item !== undefined)
    .sort(([left], [right]) => left.localeCompare(right))
  return `{${entries.map(([key, item]) => `${JSON.stringify(key)}:${sharedStableSerialize(item)}`).join(',')}}`
}
...
export function commandResultFromSlice(input: {
  commandId?: string
  status?: CommandStatus
  entity?: EntityRef
  entities?: EntityRef[]
  ...
  return {
    commandId: input.commandId || 'slice',
    status: input.status || (input.error ? 'failed' : 'completed'),
    entities: input.entities || (input.entity ? [input.entity] : []),
