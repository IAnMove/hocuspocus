...
const NARRATIVE_SCENE_TEMPLATE_DATA: Array<Omit<NarrativeSceneTemplate, 'constraints' | 'previewPrompt' | 'createScene'>> = [
  { id: 'inner-thought', title: 'Inner thought', description: 'Space for a voice-over and a contemplative drifting subject.', category: 'dialogue', visualIntent: 'Hold attention on a reflective subject while leaving visual space for narration.', referenceMotion: 'Slow lateral drift with a subtle breathing bob.', evaluationCues: ['Subject remains readable throughout.', 'Motion is calm enough for voice-over.', 'Late frames still show visible movement.'], defaultDuration: 10, assetSlots: [{ id: 'hero', label: 'Character', types: ['model3d', 'image', 'video'], required: true }, { id: 'plate', label: 'Background', types: ['image', 'video'], required: true }], controls: ['mood', 'intensity', 'voiceSpace', 'camera'] },
  ...
  { id: 'silent-reaction', title: 'Silent reaction', description: 'A restrained visual beat for listening or resolve.', category: 'dialogue', visualIntent: 'Make a small change in posture or framing carry the emotional beat.', referenceMotion: 'Minimal drift with a tiny headroom and scale change.', evaluationCues: ['Change is subtle but visible.', 'Face or focal area stays unobstructed.', 'Grade supports the emotional temperature.'], defaultDuration: 10, assetSlots: [{ id: 'hero', label: 'Character', types: ['model3d', 'image', 'video'], required: true }, { id: 'plate', label: 'Background', types: ['image', 'video'], required: true }], controls: ['mood', 'intensity', 'camera'] },
  ...
  { id: 'memory-drift', title: 'Memory / flashback', description: 'A slow visual recollection with a changing grade.', category: 'transition', visualIntent: 'Signal recollection through softness, desaturation and reverse drift.', referenceMotion: 'Slow cross-frame drift with a gentle memory-like bob.', evaluationCues: ['Grade clearly separates the memory state.', 'Motion feels slower than a present-time shot.', 'Subject remains identifiable despite softness.'], defaultDuration: 10, assetSlots: [{ id: 'hero', label: 'Character or object', types: ['model3d', 'image', 'video'], required: true }, { id: 'plate', label: 'Background', types: ['image', 'video'], required: true }], controls: ['mood', 'palette', 'voiceSpace'] },
